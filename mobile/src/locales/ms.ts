const ms = {
  common: {
    appName: 'MOE SERPS',
    logout: 'Log Keluar',
    signOut: 'Log Keluar',
    language: 'Bahasa',
    loading: 'Memuatkan...',
    noData: 'Tiada data',
    back: 'Kembali',
    save: 'Simpan',
    cancel: 'Batal',
    error: 'Ralat',
    success: 'Berjaya',
    total: 'Jumlah',
    email: 'E-mel',
    unknown: 'Tidak Diketahui',
  },
  auth: {
    username: 'Nama Pengguna',
    password: 'Kata Laluan',
    loginTitle: 'MOE SERPS',
    loginSubtitle: 'Sistem Perancangan Sumber Perusahaan Sekolah',
    loginButton: 'Log Masuk',
    loginError: 'Nama pengguna atau kata laluan tidak sah',
    accountFilled: 'Akaun diisi',
    orSignInWith: 'atau log masuk dengan',
    quickAccess: 'Akses Pantas',
    showDemoAccounts: 'Tunjukkan Akaun Demo',
    demoAccounts: 'Akaun Demo',
    clickToFill: '— ketuk baris untuk mengisi —',
    signInWithBruneiId: 'ID Digital Brunei',
    signInWithEgnc: 'EGNC / IDPM',
    ssoConsentMessage: 'Benarkan {{app}} mengakses maklumat profil anda?',
    ssoSelectDemoAccount: 'Pilih akaun demo:',
    ssoAllow: 'Benarkan {{provider}}',
    ssoRedirecting: 'Mengarahkan ke {{provider}}...',
  },
  parent: {
    title: 'Portal Ibu Bapa',
    myChildren: 'Anak-Anak Saya',
    grades: 'Markah',
    attendance: 'Kehadiran',
    gradeLevel: 'Tahun',
    gpa: 'GPA',
    attendanceRate: 'Kehadiran',
    viewGrades: 'Lihat Markah',
    viewAttendance: 'Lihat Kehadiran',
    childGrades: 'Markah Anak',
    childAttendance: 'Kehadiran Anak',
    selectChild: 'Pilih Anak',
    noChildren: 'Tiada anak dikaitkan dengan akaun anda',
    subject: 'Subjek',
    score: 'Skor',
    grade: 'Gred',
    date: 'Tarikh',
    status: 'Status',
    sessionTopic: 'Topik',
    giveReason: 'Berikan sebab',
    fees: 'Yuran',
    totalDue: 'Jumlah Perlu Bayar',
    paid: 'Dibayar',
    outstanding: 'Tertunggak',
    feeType: 'Jenis',
    feeAmount: 'Amaun',
    feeStatus: 'Status',
    semester: 'Semester',
    noFees: 'Tiada rekod yuran',
    feeSummary: 'Ringkasan Yuran',
    payNow: 'Bayar Sekarang',
    invoiceNo: 'Invois',
    homework: 'Kerja Rumah',
    noHomework: 'Tiada kerja rumah',
    homeworkType: 'Jenis',
    homeworkDue: 'Tarikh Akhir',
    submittedBy: 'Dihantar oleh',
    graded: 'Telah Dinilai',
    lateSubmission: 'Hantar Lewat',
    behavior: 'Rekod Tingkah Laku',
    parentNotified: 'Ibu Bapa Diberitahu',
    meetings: 'Mesyuarat Ibu Bapa',
    bookMeeting: 'Tempah Mesyuarat',
    selectTeacher: 'Pilih Guru',
    selectDate: 'Pilih Tarikh',
    selectTimeSlot: 'Slot Masa',
    meetingPurpose: 'Tujuan',
    confirmBooking: 'Sahkan Tempahan',
    scheduled: 'Dijadualkan',
    completed: 'Selesai',
    cancelled: 'Dibatalkan',
    noMeetings: 'Tiada mesyuarat dijadualkan',
    consentForms: 'Borang Persetujuan',
    noConsentForms: 'Tiada borang persetujuan',
    acknowledge: 'Sahkan',
    acknowledged: 'Telah Disahkan',
    pendingAck: 'Menunggu Pengesahan',
    overdue: 'Lewat Tarikh',
    formType: 'Jenis',
    dueDate: 'Tarikh Akhir',
    contactDirectory: 'Direktori Hubungan',
    principals: 'Pengetua',
    administrators: 'Pentadbir',
    counselors: 'Kaunselor',
    teachers: 'Guru',
    classTeacher: 'Guru Kelas',
    call: 'Hubungi',
    email: 'E-mel',
    noContacts: 'Tiada hubungan tersedia',
    commHistory: 'Sejarah Komunikasi',
    allEvents: 'Semua Acara',
    messages: 'Mesej',
    announcements: 'Pengumuman',
    consents: 'Persetujuan',
    events: 'Acara',
    noEvents: 'Tiada acara komunikasi',
    application: 'Permohonan',
    newApplication: 'Permohonan Baharu',
    applications: 'Permohonan Saya',
    noApplications: 'Tiada rekod permohonan',
    applicationStatus: 'Status',
    submitted: 'Dihantar',
    underReview: 'Dalam Semakan',
    approved: 'Diluluskan',
    rejected: 'Ditolak',
  },
  student: {
    title: 'Portal Pelajar',
    home: 'Utama',
    profile: 'Profil',
    courses: 'Kursus',
    grades: 'Markah',
    welcome: 'Selamat kembali',
    enrolledCourses: 'Kursus',
    attendanceRate: 'Kehadiran',
    gpa: 'GPA',
    upcomingAssessments: 'Penilaian Akan Datang',
    noUpcoming: 'Tiada penilaian akan datang',
    studentId: 'No. Pelajar',
    gradeLevel: 'Tahun',
    className: 'Kelas',
    dob: 'Tarikh Lahir',
    gender: 'Jantina',
    nationality: 'Warganegara',
    enrollmentStatus: 'Status',
    noCourses: 'Tiada kursus didaftar',
    listView: 'Senarai',
    timetableView: 'Jadual',
    allTypes: 'Semua',
    examType: 'Peperiksaan',
    quizType: 'Kuiz',
    assignmentType: 'Tugasan',
    projectType: 'Projek',
    room: 'Bilik',
    courseCode: 'Kod',
    creditHours: 'Kredit',
    schedule: 'Jadual',
    semester: 'Semester',
    teacher: 'Guru',
    noGrades: 'Tiada markah',
    overallGpa: 'GPA Keseluruhan',
    assessmentName: 'Penilaian',
    type: 'Jenis',
    maxScore: 'Markah Penuh',
    letterGrade: 'Gred',
    dueDate: 'Tarikh Akhir',
    assignments: 'Tugasan',
    submitAssignment: 'Hantar',
    pending: 'Menunggu',
    assignmentSubmitted: 'Dihantar',
    overdue: 'Lewat Tarikh',
    noStudentAssignments: 'Tiada tugasan diberikan',
    behavior: 'Tingkahlaku',
    merits: 'Jasa Baik',
    demerits: 'Kesalahan',
    netPoints: 'Mata Bersih',
    behaviorRecord: 'Rekod',
    behaviorCategory: 'Kategori',
    behaviorPoints: 'Mata',
    behaviorAction: 'Tindakan Diambil',
    behaviorDate: 'Tarikh',
    behaviorSeverity: 'Keterukan',
    noBehaviorRecords: 'Tiada rekod tingkah laku',
    reportCard: 'Kad Laporan',
    selectSemester: 'Pilih Semester',
    gradeItem: 'Item Gred',
    itemWeight: 'Berat',
    percentage: 'Peratusan',
    printReportCard: 'Cetak Kad Laporan',
  },
  teacher: {
    title: 'Portal Guru',
    home: 'Utama',
    classes: 'Kelas',
    timetable: 'Jadual',
    attendance: 'Kehadiran',
    welcome: 'Selamat kembali',
    myCourses: 'Kursus Saya',
    myStudents: 'Pelajar',
    staffId: 'No. Staf',
    department: 'Jabatan',
    designation: 'Jawatan',
    qualification: 'Kelayakan',
    noClasses: 'Tiada kursus ditugaskan',
    courseCode: 'Kod',
    creditHours: 'Kredit',
    gradeLevel: 'Tahun',
    sessions: 'Sesi',
    noSessions: 'Tiada sesi kehadiran',
    sessionDate: 'Tarikh',
    sessionStatus: 'Status',
    topic: 'Topik',
    recordCount: 'Rekod',
    active: 'Aktif',
    completed: 'Selesai',
    startAttendance: 'Mula Kehadiran',
    selectCourse: 'Pilih Kursus',
    markAttendance: 'Tandakan Kehadiran',
    saveAttendance: 'Simpan Kehadiran',
    attendanceSaved: 'Kehadiran disimpan',
    noStudentsEnrolled: 'Tiada pelajar dalam kursus ini',
    monday: 'Isnin',
    tuesday: 'Selasa',
    wednesday: 'Rabu',
    thursday: 'Khamis',
    friday: 'Jumaat',
    noClassesOnDay: 'Tiada kelas',
    grades: 'Pengurusan Gred',
    gradesTab: 'Gred',
    gradeItems: 'Item Gred',
    addGradeItem: 'Tambah Item',
    noGradeItems: 'Tiada item gred lagi. Cipta kuis atau tugasan pertama anda.',
    gradesSaved: 'Gred berjaya disimpan',
    gradeItemCreated: 'Item gred telah dicipta',
    quizNamePlaceholder: 'cth. Kuis 1 - Algebra',
    maxScore: 'Markah Penuh',
    weight: 'Berat (%)',
    type: 'Jenis',
    messages: 'Mesej',
    profile: 'Profil',
    subjects: 'Subjek',
    joinDate: 'Tarikh Mula',
    monthlyAttend: 'Kehadiran Bulanan',
    cpdHours: 'Jam CPD',
    noMessages: 'Tiada mesej daripada ibu bapa lagi',
    replyPlaceholder: 'Taip balasan...',
    sent: 'Dihantar',
    messageDetail: 'Perbualan',
    personalInfo: 'Mlumat Peribadi',
    courseAssignments: 'Tugasan Kursus',
    totalStudents: 'Jumlah Pelajar',
    activeCourses: 'Kursus Aktif',
    assignments: 'Tugasan',
    createAssignment: 'Cipta Tugasan',
    assignmentDueDate: 'Tarikh Akhir',
    submitted: 'Dihantar',
    submissions: 'Penghantaran',
    noAssignments: 'Tiada tugasan lagi',
    assignmentTitle: 'Tajuk',
    newAssignment: 'Tugasan Baharu',
    viewAttendanceHistory: 'Lihat Sejarah Kehadiran Penuh',
  },
  attendance: {
    present: 'Hadir',
    absent: 'Tidak Hadir',
    late: 'Lewat',
    excused: 'Dimaafkan',
  },
  chat: {
    title: 'Pembantu AI',
    placeholder: 'Tanya apa sahaja...',
    send: 'Hantar',
    thinking: 'Berfikir...',
    error: 'Gagal mendapat respons. Sila cuba lagi.',
    minimize: 'Minimumkan',
    quickQuestions: 'Soalan Pantas',
  },
  notifications: {
    title: 'Pemberitahuan',
    noNotifications: 'Tiada pemberitahuan',
    markAllRead: 'Tandakan semua dibaca',
    empty: 'Anda telah membaca semuanya!',
  },
  course: {
    detail: 'Perincian Kursus',
    creditHours: 'Kredit',
    enrolledStudents: 'Pelajar Berdaftar',
    noStudents: 'Tiada pelajar berdaftar',
    details: 'Butiran',
    code: 'Kod',
    gradeLevel: 'Tahun',
    hours: 'jam',
  },
  announcements: {
    title: 'Pengumuman',
    noAnnouncements: 'Tiada pengumuman',
    viewAll: 'Lihat Semua',
    pinned: 'Disemat',
    high: 'Tinggi',
    normal: 'Biasa',
    urgent: 'Segera',
    detail: 'Perincian Pengumuman',
    publishedBy: 'Diterbitkan oleh',
  },
  messages: {
    title: 'Mesej',
    noMessages: 'Tiada mesej lagi',
    sendError: 'Gagal menghantar mesej. Sila cuba lagi.',
    inputPlaceholder: 'Taip mesej...',
    detail: 'Perbualan',
    subject: 'Subjek',
    teacher: 'Guru',
    parent: 'Ibu Bapa',
  },
  checkIn: {
    title: 'Kehadiran Hari Ini',
    checkIn: 'Daftar Masuk',
    checkOut: 'Daftar Keluar',
    checkedIn: 'Telah daftar masuk pada',
    checkedOut: 'Telah daftar keluar pada',
    notCheckedIn: 'Belum daftar masuk hari ini',
    allDone: 'Kehadiran hari ini selesai',
    confirmCheckIn: 'Sahkan daftar masuk sekarang?',
    confirmCheckOut: 'Sahkan daftar keluar sekarang?',
    checkInSuccess: 'Berjaya daftar masuk',
    checkOutSuccess: 'Berjaya daftar keluar',
    failed: 'Gagal, sila cuba lagi',
    late: 'Lewat',
    present: 'Tepat masa',
    weekend: 'Tiada kehadiran diperlukan hari ini',
  },
  push: {
    enableTitle: 'Kekal Dikemaskini',
    enableDesc: 'Aktifkan pemberitahuan tolak untuk menerima amaran apabila kehadiran anak anda direkodkan.',
    enableBtn: 'Aktifkan Pemberitahuan',
    disableBtn: 'Nyahaktifkan Pemberitahuan',
    testBtn: 'Hantar Ujian',
    enabled: 'Pemberitahuan diaktifkan',
    denied: 'Pemberitahuan disekat. Sila benarkan dalam tetapan pelayar.',
    unsupported: 'Pemberitahuan tolak tidak disokong dalam pelayar ini.',
    offlineBanner: 'Anda sedang luar talian. Data yang ditunjukkan mungkin tidak terkini.',
    offlineAttendanceQueued: 'Kehadiran disimpan luar talian. Akan disegerakkan apabila disambungkan.',
  },
}

export default ms
