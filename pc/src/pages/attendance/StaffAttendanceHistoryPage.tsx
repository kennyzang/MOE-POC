...
import {
  Card, Row, Col, Typography, Space, Tag, Table, Segmented, Statistic, Spin,
} from 'antd'
import type { ColumnsType } from 'antd/es/table'
import { useTranslation } from 'react-i18next'
import { useQuery } from '@tanstack/react-query'
import { CalendarDays, Clock, TrendingUp, XCircle } from 'lucide-react'
...
/** Simple 6-week calendar grid (last 42 days) */
...
export default function StaffAttendanceHistoryPage() {
  ...
  const { data, isLoading } = useQuery<HistoryData>({
    queryKey: ['staffAttendanceHistory', period],
    queryFn: async () => {
      const r = await api.get(`/staff-attendance/history?period=${period}`)
      return r.data.data as HistoryData
    },
  })
  ...
  return (
    <div style={{ padding: 24 }}>
      <Space style={{ marginBottom: 24 }} align="center">
        ...
        <Title level={4} style={{ margin: 0 }}>{t('attendance.myHistory', 'My Attendance History')}</Title>
      </Space>
